...
class ListItemsControllerTest < ActionDispatch::IntegrationTest
  setup do
    @list_item = list_items(:one) # Assicurati di avere un fixture chiamato :one
  end
  ...
  test "should create list_item" do
    assert_difference("ListItem.count") do
      post list_items_url, params: { list_item: { title: "Nuovo Item", content: "Descrizione di test" } }
    end
  ...
  test "should update list_item" do
    patch list_item_url(@list_item), params: { list_item: { title: "Titolo aggiornato" } }
